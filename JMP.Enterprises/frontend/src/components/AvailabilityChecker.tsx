  ...
  return (
    <div className="card" style={{ marginBottom: '24px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '16px' }}>
        ...
        <h3 style={{ fontSize: '1.05rem', fontWeight: 700 }}>Property Availability Checker</h3>
      </div>
      ...
      {results && (
        <div style={{ borderTop: '1px solid var(--border-color)', paddingTop: '16px', marginTop: '16px' }}>
          <div style={{ fontSize: '0.8rem', fontWeight: 600, color: 'var(--text-secondary)', marginBottom: '12px' }}>
            Availability Results for {new Date(checkIn).toLocaleDateString()} to {new Date(checkOut).toLocaleDateString()}:
          </div>
                ...
                <div>
                  ...
                  {!item.isAvailable && item.currentReservationGuest && (
                    <div style={{ fontSize: '0.75rem', color: 'var(--danger-text)', marginTop: '4px' }}>
                      Reserved by: <strong>{item.currentReservationGuest}</strong>
                    </div>
                  )}
                </div>
